...
var actions = {
  ...
  'repeat': {
    permission: 'control',
    fn: function(client, msg, cb) {
      this.player.setRepeatOn(!!msg.repeat);
      this.player.setRepeatSingle(!!msg.single);
      cb({});
    },
  },
  'seek': {
  ...
  'status': {
    ...
    fn: function(client, msg, cb) {
      cb({msg: {
        volume: null,
        repeat: this.player.repeat.repeat,
        single: this.player.repeat.single,
...
PlayerServer.prototype.createClient = function(socket, permissions) {
  ...
  self.player.on('status', function(arg){
    try {
