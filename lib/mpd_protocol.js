...
var GROOVEBASIN_PROTOCOL_UUID = "OHqIjsp9g9dpGRZ3p8LcxVey9tpMh_bT";
...
var commands = {
  ...
  "listplaylist": {
    ...
    fn: function (self, args, cb) {
      ...
      plItems.forEach(function(plItem) {
        var track = self.player.library.trackTable[plItem.key];
        self.push("file: " + track.file + "\n");
      });
      cb();
    },
  ...
  "load": {
    ...
    fn: function (self, args, cb) {
      ...
      var start = 0;
      var end = Object.keys(playlist.items).length;
      ...
      var plItems = playlistItemsInOrder(playlist);
      var trackKeys = plItems.slice(start, end);
  ...
  "playlistadd": {
    ...
    fn: function (self, args, cb) {
      ...
      var plItems = playlistItemsInOrder(playlist);
      var lastSortKey = plItems[plItems.length - 1].sortKey;
  ...
  "playlistclear": {
    ...
    fn: function (self, args, cb) {
      ...
      var itemKeys = Object.keys(playlist.items);
      self.playerServer.playlistRemoveItems(self.client, playlist.id, itemKeys);
      cb();
    }
  },
  "playlistdelete": {
    ...
    fn: function (self, args, cb) {
      ...
      self.playerServer.playlistRemoveItems(self.client, playlist.id, [plItem.id]);
      cb();
    }
  },
  "playlistid": {
  ...
  "playlistmove": {
    ...
    fn: function (self, args, cb) {
      ...
      self.playerServer.playlistMoveItems(self.client, playlist.id, items);
      cb();
    }
  },
  "plchanges": {
...
MpdProtocol.prototype.upgradeProtocol = function() {
  ...
  self.mpdTransport.emit('close');
  self.mpdTransport = null;
  self.client = self.playerServer.createClient(self.grooveBasinProtocol, "GB-Guest");
...
function findOrSearchAdd(self, args, caseSensitive, cb) {
  ...
  function onFinish() {
    self.player.queueTrackKeys(keys);
    cb();
  }
}
...
function addCmd(self, args, cb) {
  ...
  if (dbFile) {
    self.player.queueTrackKeys([dbFile.key]);
    cb();
    return;
  }
