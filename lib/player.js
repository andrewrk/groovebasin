...
function Player(db, config) {
  ...
  this.groovePlaylist.setFillMode(groove.EVERY_SINK_FULL);
...
function startPlayerSwitchDevice(self, wantHardware, cb) {
  ...
  function onDetachComplete(err) {
    ...
    if (wantHardware) {
      ...
      groove.connectSoundBackend(groove.BACKEND_DUMMY);
    }
    var devices = groove.getDevices();
    self.groovePlayer.device = devices.list[devices.defaultIndex];
...
Player.prototype.performScan = function(args, cb) {
  ...
  dbFilesToOpen.forEach(function(dbFile) {
    pend.go(function(cb) {
      ...
      groove.open(fullPath, function(err, file) {
        if (dbFile.duration) {
          file.overrideDuration(dbFile.duration);
        }
        if (err) {
  ...
  var scanWaveform;
  var scanWaveformAttached = false;
  var endOfWaveformCb;
  ...
  function attachLoudnessDetector(cb) {
    scanDetector.attach(scanPlaylist, function(err) {
      ...
      endOfDetectorCb = endOfPlaylistPend.hold();
      cb();
    });
  }
  ...
  function attachFingerprinter(cb) {
    scanFingerprinter.attach(scanPlaylist, function(err) {
      ...
      endOfFingerprinterCb = endOfPlaylistPend.hold();
      cb();
    });
  }

  function attachWaveform(cb) {
    scanWaveform.attach(scanPlaylist, function(err) {
      if (err) return cb(err);
      scanWaveformAttached = true;
      endOfWaveformCb = endOfPlaylistPend.hold();
      cb();
    });
  }
  ...
  function onWaveformInfo() {
    var info;
    while (info = scanWaveform.getInfo()) {
      if (info.item) {
        var fileInfo = files[info.item.file.id];
        fileInfo.waveformDone = true;
        var dbFile = fileInfo.dbFile;
        log.info("waveform scan file complete:", dbFile.name);
        dbFile.waveform = info.buffer;
        self.emit('scanProgress');
      } else {
        log.debug("waveform scan complete:", JSON.stringify(scanKey));
        if (endOfWaveformCb) {
          endOfWaveformCb();
          endOfWaveformCb = null;
        }
        return;
      }
    }
  ...
  function cleanupAndCb() {
    ...
    if (scanWaveformAttached) pend.go(detachWaveform);
    pend.wait(function(err) {
      if (scanPlaylist) scanPlaylist.destroy();
  ...
  function detachWaveform(cb) {
    scanWaveform.detach(cb);
  }
};
...
function trackWithoutIndex(category, dbFile) {
  ...
  for (var propName in DB_PROPS) {
    ...
    if (prop.type === 'set') {
      ...
      out[propName] = Array.prototype.slice.call(value, 0);
    } else {
      out[propName] = value;
    }
  }
