...
GrooveBasin.prototype.initializeUpload = function() {
  ...
  self.app.post('/upload', multipart, function(request, response) {
    ...
    pend.wait(function() {
      response.json(completedKeys);
    });

    function makeImportFn(filePath, originalFilename) {
      return function(cb) {
        self.player.importFile(filePath, originalFilename, function(err, dbFile) {
          if (err) {
            console.error("Unable to import by uploading. Error:", err.stack);
          } else {
            completedKeys.push(dbFile.key);
          }
          cb();
        });
      };
...
function genPassword() {
  return crypto.pseudoRandomBytes(8).toString('base64');
}
