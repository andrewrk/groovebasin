    ...
    groove.createPlayer(function(err, player) {
      ...
      self.player.on('nowplaying', function() {
        ...
        if (obj.item) {
          ...
          console.info("nowplaying event -> currentTrack = null");
...
function checkUpdateGroovePlaylist(self) {
  console.info("     begin checkUpdateGroovePlaylist");
  if (!self.currentTrack) {
    console.info("checkUpdateGroovePlaylist: empty");
    self.player.clear();
    self.grooveItems = {};
    return;
  }
  ...
  while (groovePlIndex < groovePlaylist.length) {
    ...
    if (grooveTrack.index < self.plItemIndex) {
      console.info("checkUpdateGroovePlaylist: deleting old entry:", grooveTrack.file);
    ...
    if (grooveTrack === plTrack) {
      ...
      console.info("checkUpdateGroovePlaylist: keeping good entry:", grooveTrack.file);
      if (!currentGrooveItem) {
        console.info("checkUpdateGroovePlaylist: noticing that our current item is:",
            grooveTrack.file);
      }
    ...
    console.info("checkUpdateGroovePlaylist: deleting wrong entry:", grooveTrack.file);
    self.player.remove(grooveItem);
    delete self.grooveItems[grooveItem.id];
    groovePlIndex += 1;
  }
  ...
  while (plItemIndex < self.currentTrack.index + NEXT_FILE_COUNT) {
    ...
    if (!plTrack || !plTrack.grooveFile) {
      console.info("checkUpdateGroovePlaylist: next track is nonexistent or waiting for file load");
      // we can't do anything
      break;
    }
    console.info("checkUpdateGroovePlaylist: inserting entry:", plTrack.file);
  ...
  console.info("checkUpdateGroovePlaylist: playlist:",
      self.player.playlist().map(function(item) { return self.grooveItems[item.id]; }));

  if (self.seekRequestPos >= 0 && currentGrooveItem) {
    var seekPos = self.seekRequestPos;
    console.log("seek", self.grooveItems[currentGrooveItem.id], "pos", seekPos);
