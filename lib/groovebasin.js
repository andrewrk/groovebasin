  ...
  function sendZipOfFiles(zipName, files, req, resp) {
    ...
    function addOne() {
      ...
      readStream.on('error', function(err) {
        log.error("zip read stream error:", err.stack);
        cleanupEverything();
      });
      cleanup.push(function() {
        readStream.destroy();
      ...
      archive.entry(readStream, options, function(err) {
        if (err) {
          log.error("Error adding zip entry:", err.stack);
          cleanupEverything();
          return;
        }
        fileIndex += 1;
    ...
    function finalize() {
      archive.finalize(function(err) {
        if (err) {
          log.error("Error finalizing zip:", err.stack);
          cleanupEverything();
        }
      });
    }
...
GrooveBasin.prototype.initializeStream = function() {
  ...
  self.app.get('/stream.mp3', self.hasPermRead, function(req, resp, next) {
    ...
    req.on('close', function() {
      ...
      var streamerCount = 0;
      for (var clientId in self.playerServer.clients) {
        var client = self.playerServer.clients[clientId];
        streamerCount += client.streaming;
      }
      if (self.player.openStreamers.length === 0 && streamerCount === 0) {
        self.player.lastStreamerDisconnected();
      }
    });
  });
};
