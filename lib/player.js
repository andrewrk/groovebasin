...
Player.prototype.delDbEntry = function(dbFile) {
  ...
  self.emit('deleteDbTrack', dbFile);
...
Player.prototype.checkDynamicMode = function() {
  ...
  for (var i = 0; i < deleteCount; i += 1) {
    delete self.playlist[self.tracksInOrder[i].id];
    didAnything = true;
  }

  var keys = getRandomSongKeys(addCount);
  if (keys.length > 0) didAnything = true;
  if (didAnything) self.appendTracks(keys, true);
...
function grooveFileToDbFile(file, filenameHint, object) {
  ...
  object.track = parsedTrack.track;
  object.trackCount = parsedTrack.trackCount;
  object.disc = parsedDisc.disc;
  object.discCount = parsedDisc.discCount;
