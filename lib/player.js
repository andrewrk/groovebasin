...
Player.prototype.initialize = function(cb) {
  ...
  function cacheAllLabels(cb) {
    ...
    function processOne(key, value) {
      ...
      self.labels[labelId] = labelEntry;
    }
  }
  ...
  function cacheAllDb(cb) {
    ...
    function processOne(key, value) {
      ...
      if (self.dbFilesByPath[dbFile.file]) {
        ...
        for (var i = 0; i < dbFile.labels.length; i++) {
          var labelId = dbFile.labels[i];
...
Player.prototype.labelCreate = function(id, name) {
  ...
  // TODO: sanitize name
  var labelEntry = {name: name};
  this.labels[id] = labelEntry;
...
Player.prototype.labelDelete = function(ids) {
  ...
  for (var i = 0; i < ids.length; i++) {
    var labelId = ids[i];
    if (!(labelId in this.labels)) continue;
    ...
    for (var fileId in files) {
      ...
      dbFile.labels.splice(dbFile.labels.indexOf(labelId), 1);
      persistDbFile(dbFile, updateCmds);
      libraryChanged = true;
    }
...
Player.prototype.changeLabels = function(changes, isAdd) {
  ...
  for (var id in changes) {
    ...
    for (var i = 0; i < labelIds.length; i++) {
      ...
      if (isAdd) {
        if (dbFile.labels.indexOf(labelId) !== -1) continue; // already got it
        dbFile.labels.push(labelId);
        if (filesByThisLabel == null) filesByThisLabel = self.dbFilesByLabel[labelId] = {};
        filesByThisLabel[dbFile.key] = dbFile;
      } else {
        var index = dbFile.labels.indexOf(labelId);
        if (index === -1) continue; // already gone
        dbFile.labels.splice(index, 1);
