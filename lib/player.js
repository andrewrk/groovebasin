...
function Player(db, musicDirectory, encodeQueueDuration, googleApiKey, bitRate, encodeFormat) {
  EventEmitter.call(this);
  this.setMaxListeners(0);

  this.selectedEncodeFormat = recognizedEncodeFormats[encodeFormat];
  if (!this.selectedEncodeFormat) {
    console.error("unrecognized encode format: " + encodeFormat);
    console.error("recognized encode formats are: " + Object.keys(recognizedEncodeFormats).join(" "));
    process.exit(1);
  }
  ...
  this.grooveEncoder.formatShortName = this.selectedEncodeFormat.format;
  this.grooveEncoder.codecShortName = this.selectedEncodeFormat.codec;
  this.grooveEncoder.mimeType = this.selectedEncodeFormat.mime;
...
Player.prototype.getStreamEndpoint = function() {
  return "/" + this.getStreamUrl();
};

Player.prototype.getStreamUrl = function() {
  return "stream." + this.selectedEncodeFormat.extension;
};

Player.prototype.getStreamMimeType = function() {
  return this.selectedEncodeFormat.mime;
};
