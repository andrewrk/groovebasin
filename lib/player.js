...
Player.prototype.refreshFilesIndex = function(args, cb) {
  ...
  function logIfError(err) {
    if (err) {
      log.error("Unable to write detected deletions to disk:", err.stack);
    }
  }
};
...
Player.prototype.playlistAddItems = function(playlistId, items) {
  ...
  if (updateCmds.length > 0) {
    ...
    this.db.batch(updateCmds, logIfError);
    this.emit('playlistUpdate', playlist);
  }

  function logIfError(err) {
    if (err) {
      log.error("Error updating playlist entries in db:", err.stack);
    }
  }
};
...
Player.prototype.playlistMoveItems = function(playlistId, items) {
  ...
  for (var id in items) {
    ...
    if (!playlistItem) continue; // race conditions, etc.

    playlistItem.sortKey = items[id].sortKey;
    updateCmds.push({
      type: 'put',
      key: playlistItemKey(playlist, playlistItem),
      value: serializePlaylistItem(playlistItem),
