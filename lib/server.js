var fs, http, net, socketio, socketio_client, express, path, assert, mkdirp, PlayerClient, MpdParser, PlayerServer, async, which, Library, MpdConf, Killer, spawn, EventEmitter, ref$, HOST, PORT, RUN_DIR, MPD_SOCKET_PATH, STATE_FILE, MPD_CONF_PATH, MPD_PID_FILE, mpd_conf, player_server, my_player, state, app, io, plugins, library, STATE_VERSION, DEFAULT_PERMISSIONS, saveState, connectToMpd, connectMasterPlayer;
fs = require('fs');
http = require('http');
net = require('net');
...
PORT = parseInt(process.env.PORT, 10) || 16242;
RUN_DIR = "run";
...
library = new Library(plugins.bus);
...
saveState = oncePerEventLoopFunc(function(cb){
  plugins.bus.emit('save_state', state);
...
connectToMpd = makeConnectFunction('mpd', {
...
connectMasterPlayer = makeConnectFunction('master player', {
  createSocket: function(){
...
async.series([initState, makeRunDir, bind$(plugins, 'initialize'), restoreState, writeMpdConf, restartMpd], function(err){
  assert.ifError(err);
  connectToMpd();
});
