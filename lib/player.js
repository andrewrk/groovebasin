  ...
  groove.open(fullPath, function(err, file) {
    ...
    pend.wait(function(err) {
      ...
      mv(fullPath, newPath, {mkdirp: true}, function(err) {
        ...
        self.markAlbumForScanning(newDbFile);
...
Player.prototype.markAlbumForScanning = function(dbFile) {
  var albumKey = this.libraryIndex.getAlbumKey(dbFile);
  console.log("marking for scanning:", JSON.stringify(albumKey));
  this.scanQueue.enqueue(albumKey, null);
};

function performScan(albumKey, _, cb) {
  var self = this;
  ...
  console.info("Replaygain scan starting:", JSON.stringify(albumKey));
  var ongoingScanContext = self.ongoingScans[albumKey];
  ...
  self.ongoingScans[albumKey] = scanContext;
  ...
  pend.wait(function() {
    ...
    scan.on('error', function(err) {
      console.error("Error scanning", JSON.stringify(albumKey), err.stack);
    ...
    scan.on('end', function(gain, peak) {
      ...
      console.info("Replaygain scan complete:", JSON.stringify(albumKey), "gain", gain);
      delete self.ongoingScans[albumKey];
