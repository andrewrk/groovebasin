...
function performScan(albumKey, _, cb) {
  ...
  pend.wait(function() {
    if (scanContext.aborted) return cb();
    ...
    scan.on('file', function(file, gain, peak) {
      ...
      console.info("replaygain scan file complete:", fileInfo.track.name);
    ...
    scan.on('end', function(gain, peak) {
      ...
      console.info("Replaygain scan complete:", JSON.stringify(albumKey));
