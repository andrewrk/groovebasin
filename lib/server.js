...
var ClementineServer = require('./clementine_server');
...
function startServer(app, db, player, config, sslEnabled) {
  ...
  var clementineServer = new ClementineServer(player);
  ...
  function attachWebSocketServer() {
    ...
    var clementinePort = config.clementinePort;
    var clementineHost = config.clementineHost;
    if (clementinePort == null || clementineHost == null) {
      log.info("Clementine Protocol disabled");
    } else {
      clementineServer.listen(clementinePort, clementinePort, function() {
        log.info("Clementine protocol listening at " +
          clementineHost + ":" + clementinePort);
      });
    }
  }
}
