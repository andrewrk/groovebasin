...
var instantBufferBytes = 192 * 1024;
        ...
        while (1) {
          ...
          if (buf.buffer) {
            if (buf.item) {
              ...
              while (self.recentBuffersByteCount > instantBufferBytes) {
...
Player.prototype.refreshFilesIndex = function(dir, cb) {
  ...
  walker.on('end', function() {
    ...
    for (var scannedDirName in scannedDirs) {
      ...
      for (baseName in dirEntry.entries) {
        ...
        if (id !== thisScanId) {
          onFileMissing(path.join(dirEntry.dirName, baseName));
        }
      }
      for (baseName in dirEntry.dirEntries) {
        id = dirEntry.dirEntries[baseName];
        if (id !== thisScanId) {
          onDirMissing(path.join(dirEntry.dirName, baseName));
        }
      }
    }
    cb();
  ...
  function onFileMissing(relPath) {
    console.log("file deleted:", relPath);
    var dbFile = self.dbFilesByPath[relPath];
    if (!dbFile) {
      console.warn("File reported deleted that was not in our db:", relPath);
      return;
    }
  ...
  function onAddOrChange(relPath, stat) {
    ...
    console.log("added or changed file:", relPath);
...
function grooveFileToDbFile(file, filenameHint) {
  ...
  return {
    ...
    composerName: (file.getMetadata("composer") || "").trim(),
