    ...
    form.on('close', function() {
      pend.wait(function() {
        if (allDbFiles.length >= 1) {
          ...
          playerServer.addEvent({
            user: user,
            type: 'import',
            trackKey: allDbFiles[0].key,
            pos: allDbFiles.length
          });
          if (autoQueue) {
...
function startServer(app, db, player, config) {
  ...
  function attachWebSocketServer() {
    ...
    wss.on('connection', function(ws) {
      playerServer.handleNewClient(new WebSocketApiClient(ws));
    });
    ...
    if (mpdPort == null || mpdHost == null) {
      ...
      var protocolServer = net.createServer(function(socket) {
        ...
        mpdApiServer.handleNewClient(protocol);
        ...
        function cleanup() {
          mpdApiServer.handleClientEnd(protocol);
        }
      });
