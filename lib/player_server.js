...
var actions = {
    ...
    fn: function(self, client, args) {
      ...
      self.player.importUrl(urlString, function(err, dbFiles) {
        if (err) {
          ...
          self.addDbFilesToIncomingPlaylist(dbFiles);
        }
      });
    },
  },
  ...
  'playlistRemoveItems': {
    ...
    args: objectArg({
      id: stringArg(UUID_LEN),
      items: arrayArg(stringArg(UUID_LEN), MAX_KEY_COUNT),
    }),
    fn: function(self, client, args) {
      self.playlistRemoveItems(client, args.id, args.items);
    },
  },
  'playlistMoveItems': {
    permission: 'control',
    ...
    fn: function(self, client, args) {
      self.playlistMoveItems(client, args.id, args.items);
    },
  },
  'LastFmGetSession': {
    permission: 'read',
...
PlayerServer.prototype.playlistRemoveItems = function(client, playlistId, itemKeys) {
  ...
  this.addEvent({
    ...
    playlistId: playlistId,
    pos: itemKeys.length,
    trackKey: playlist.items[itemKeys[0]].key,
  });
  this.player.playlistRemoveItems(playlistId, itemKeys);
};

PlayerServer.prototype.playlistMoveItems = function(client, playlistId, items) {
  var itemCount = keyCount(items);
  if (itemCount === 0) return;
  this.addEvent({
    user: client.user,
  ...
  this.player.playlistMoveItems(playlistId, items);
};
