...
var actions = {
  ...
  'login': {
    ...
    args: 'object',
    fn: function(self, client, args) {
      if (!self.validateString(client, args.username)) return;
...
PlayerServer.prototype.createClient = function(transport, prefix) {
  ...
  function onMessage(name, args) {
    ...
    if (perm != null && !self.userHasPerm(client.user, perm)) {
      ...
      transport.sendMessage("error", errText);
      return;
    }
    ...
    if (action.args && argsType !== action.args) {
      ...
      transport.sendMessage("error", "expected " + action.args + ": " + JSON.stringify(args));
      return;
    }
...
PlayerServer.playlistRename = function(client, playlistId, newName) {
...
PlayerServer.playlistDelete = function(client, playlistIds) {
