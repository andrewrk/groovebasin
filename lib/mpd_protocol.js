...
var commands = {
  ...
  "consume": {
    ...
    fn: function (self, args, cb) {
      self.player.updateDynamicMode({on: args.state});
      cb();
    },
  },
  "count": {
...
MpdProtocol.prototype.initialize = function() {
  ...
  self.push("OK MPD 0.19.0\n");

  self.handleEvent(self.player, 'volumeUpdate', onVolumeUpdate);
  self.handleEvent(self.player, 'repeatUpdate', updateOptionsSubsystem);
  self.handleEvent(self.player, 'dynamicModeOn', updateOptionsSubsystem);
  self.handleEvent(self.player, 'playlistUpdate', onPlaylistUpdate);
  self.handleEvent(self.player, 'deleteDbTrack', updateDatabaseSubsystem);
  self.handleEvent(self.player, 'addDbTrack', updateDatabaseSubsystem);
  self.handleEvent(self.player, 'updateDbTrack', updateDatabaseSubsystem);
  self.handleEvent(self.apiServer, 'singleMode', updateOptionsSubsystem);

  self.once('finish', self.removeAllEventHandlers.bind(self));
  ...
  function subsystemUpdate(subsystem) {
    self.updatedSubsystems[subsystem] = true;
    if (self.isIdle) self.handleIdle();
  }
}

MpdProtocol.prototype.handleEvent = function(eventEmitter, eventName, handler) {
  this.eventHandlers.push([eventEmitter, eventName, handler]);
  eventEmitter.on(eventName, handler);
};

MpdProtocol.prototype.removeAllEventHandlers = function() {
  while (this.eventHandlers.length) {
    var subscription = this.eventHandlers.shift();
    var eventEmitter = subscription[0];
    var eventName = subscription[1];
    var handler = subscription[2];
