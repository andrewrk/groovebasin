...
var DbBackedQueue = require('./db_backed_queue');
...
function Player(db, musicDirectory) {
  ...
  this.ongoingScans = {};
  this.scanQueue = new DbBackedQueue({
    db: db,
    dispatch: performScan.bind(this),
    keyPrefix: "Scan.",
  });
  this.scanPend = new Pend();
  this.scanPend.max = cpuCount + 2;
}
...
Player.prototype.initialize = function(cb) {
  ...
  function initLibrary(cb) {
    cacheAllDb(function(err) {
      ...
      pend.go(self.scanQueue.flush.bind(self));
      watchLibrary();
      cb();
    });
  }
...
Player.prototype.delDbEntry = function(dbFile) {
  var self = this;
  self.markForScanning(dbFile, true);
  ...
  groove.open(fullPath, function(err, file) {
    ...
    pend.wait(function(err) {
      ...
      mv(fullPath, newPath, {mkdirp: true}, function(err) {
        ...
        self.markForScanning(newDbFile, false);
...
Player.prototype.markForScanning = function(dbFile, albumOnly) {
  var scanId = null;
  if (dbFile.albumName) {
    scanId = 'album\n' + this.libraryIndex.getAlbumKey(dbFile);
  } else if (!albumOnly) {
    scanId = 'track\n' + dbFile.key;
  }
  if (scanId) {
    console.info("marking for scanning:", JSON.stringify(scanId));
    this.scanQueue.enqueue(scanId, null);
  }
};

function splitOnce(str, separator) {
  var index = str.indexOf(separator);
  return [str.substr(0, index), str.substr(index + separator.length)];
}

function performScan(scanKey, _, done1) {
  var self = this;

  var parts = splitOnce(scanKey, "\n");
  var scanType = parts[0];
  var scanSpecificKey = parts[1];

  var ongoingScanContext = self.ongoingScans[scanKey];
  if (ongoingScanContext) {
    ongoingScanContext.aborted = true;
    if (ongoingScanContext.scan) {
      ongoingScanContext.scan.abort();
    }
    if (ongoingScanContext.timeout) {
      console.info("good thing we delayed the scan");
      clearTimeout(ongoingScanContext.timeout);
      done1();
    }
    ongoingScanContext = null;
  }
  var scanContext = {
    scan: null,
    files: {},
  ...
  self.ongoingScans[scanKey] = scanContext;
  // give a little timeout to let complete albums be added
  scanContext.timeout = setTimeout(function() {
    scanContext.timeout = null;
    self.scanPend.go(function(done2) {
      doIt(cb);
      function cb() {
        done2();
        done1();
  ...
  function doIt(cb) {
    ...
    trackList.forEach(function(track) {
      pend.go(function(cb) {
        ...
        groove.open(fullPath, function(err, file) {
          if (err) {
            console.error("Error opening", file.filename, "in order to scan:", err.stack);
          } else {
...
function checkUpdateGroovePlaylist(self) {
  ...
  while (groovePlIndex < groovePlaylist.length) {
    ...
    if (grooveTrack === plTrack) {
      ...
      self.player.setItemGain(grooveItem, calcGain(plTrack));
  ...
  while (groovePlItemCount < NEXT_FILE_COUNT) {
    ...
    grooveItem = self.player.insert(plTrack.grooveFile, calcGain(plTrack));
  ...
  function calcGain(plTrack) {
    ...
    var replayGain = dbFile.replayGain;
    if (replayGain) {
      gain *= dBToFloat(albumMode ? replayGain.albumGain : replayGain.trackGain);
    } else {
