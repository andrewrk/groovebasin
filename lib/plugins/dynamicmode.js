...
function DynamicMode(gb) {
  ...
  this.gb.player.dynamicMode = this.state;
  this.gb.player.updateDynamicMode = this.setState.bind(this);
}
...
DynamicMode.prototype.initialize = function(cb) {
  ...
  self.gb.db.get(DB_KEY, function(err, value) {
    ...
    self.setState(JSON.parse(value), true);
    cb();
  });
};

DynamicMode.prototype.setState = function(state, init) {
  ...
  if (anythingChanged && !init) {
    this.gb.player.emit('dynamicModeUpdate');
...
DynamicMode.prototype.persist = function() {
  var self = this;
  self.gb.db.put(DB_KEY, JSON.stringify(self.state), function(err) {
    if (err) {
