...
Player.prototype.importUrl = function(urlString, cb) {
  ...
  function tryImportFilter() {
    ...
    function callNextFilter(err, dlStream, filenameHintWithoutPath) {
      ...
      handleDownload(dlStream, filenameHintWithoutPath);
    }
  }
  ...
  function handleDownload(req, filenameHintWithoutPath) {
    ...
    ws.on('close', function(){
      ...
      self.importFile(ws.path, filenameHintWithoutPath, function(err, dbFile) {
        if (err) {
          cleanAndCb(err);
        } else {
          calledCallback = true;
