...
var actions = {
  ...
  'dynamicMode': {
    permission: 'control',
    fn: function(client, msg, cb) {
      this.player.updateDynamicMode(msg.value);
      cb({});
    },
  },
  'listallinfo': {
  ...
  'seek': {
    permission: 'control',
    fn: function(client, msg, cb) {
      this.player.seek(msg.id, msg.pos);
      cb({});
    },
  },
  'playlistinfo': {
...
PlayerServer.prototype.createClient = function(socket, permissions) {
  ...
  self.player.on('dynamicModeUpdate', function() {
    try {
      socket.emit('dynamicMode', self.player.dynamicMode);
    } catch (e$) {}
  });
